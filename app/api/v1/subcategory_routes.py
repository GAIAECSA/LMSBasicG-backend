...
def get_subcategory(subcategory_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def get_subcategories_by_category(category_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def get_all_subcategories(db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
