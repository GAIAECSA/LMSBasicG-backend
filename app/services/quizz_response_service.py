...
from app.repositories.certificate_repo import get_by_user_and_course as CER_get_by_user_and_course, update as CER_update
...
def update_quizz_response(db: Session, quizz_response_id: int, data: QuizzResponseUpdate):
    ...
    if update_data.get("score") is not None or update_data.get("is_passed") is not None:
        ...
        certificate = CER_get_by_user_and_course(db, quizz_response.enrollment.user_id, quizz_response.enrollment.course_id)
        if certificate:
...
def get_by_enrollment(db: Session, enrollment_id: int):
    ...
    return [
        ...
        if (
            lesson_block := LES_BLO_get_by_id(
                db,
...
def calculate_final_grade_average(quizz_responses: list[QuizzResponse]) -> float | None:
    ...
    total_score = sum(quizz_response.score for quizz_response in quizz_responses if quizz_response.score is not None)
    count = sum(1 for quizz_response in quizz_responses if quizz_response.score is not None)
