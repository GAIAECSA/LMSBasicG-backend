...
def create_attendance(db: Session,data: AttendanceCreate):
    existing = attendance_repo.get_by_enrollment_and_course_attendance(db,data.enrollment_id,data.course_attendance_id)
    if existing:
...
def update_attendance(db: Session,attendance_id: int,data: AttendanceUpdate):
    ...
    course_attendance_id = update_data.get("course_attendance_id",attendance.course_attendance_id)
