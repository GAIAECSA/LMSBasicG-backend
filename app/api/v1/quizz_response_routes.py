...
def create_quizz_response(data: QuizzResponseCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def update_quizz_response(quizz_response_id: int, data: QuizzResponseUpdate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def get_by_enrollment(enrollment_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def get_by_block(lesson_block_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def get_quizz_response(quizz_response_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    try:
...
def delete_quizz_response(quizz_response_id: int, db: Session = Depends(get_db), user=Depends(require_admin)):
    try:
