...
class MDTFileNotFoundError(Exception):
    pass


class MDTCertificateNotFoundError(Exception):
    pass


class IdNumberNotFoundError(Exception):
    pass
...
def create_certificate(
    ...
    with db.begin():
        if not file:
            raise MDTFileNotFoundError("El archivo es requerido")
        ...
        saved_file = save_certificate_mdt(file, business_id)

        certificate_data["file_url"] = saved_file["file_url"]
        certificate_data["file_name"] = saved_file["filename"]
        ...
        if existing_certificate:
            ...
            for key, value in certificate_data.items():
                setattr(existing_certificate, key, value)

            return existing_certificate

        new_certificate = MdtCertificate(**certificate_data, business_id=business_id)

        mdt_certificate_repo.create(new_certificate)
...
def create_certificates_bulk(
    ...
    if not files:
        raise FileNotFoundError("Debe enviar archivos")
    ...
    certificate_type = base_data["certificate_type"]
        ...
        for index, file in enumerate(files):
            try:
                with db.begin_nested():
                    ...
                    match = re.search(ID_NUMBER_REGEX, file.filename)
                    if not match:
                        raise IdNumberNotFoundError(
                    ...
                    if existing_certificate:
                        ...
                        mdt_certificate_repo.create(certificate)
...
def get_certificate_by_id(db: Session, certificate_id: int, business_id: int):

    certificate = mdt_certificate_repo.get_by_id(db, certificate_id, business_id)

    if not certificate:
        raise MDTCertificateNotFoundError("Certificado no encontrado")

    return certificate


def get_certificates_by_course_id(db: Session, course_id: int, business_id: int):

    return mdt_certificate_repo.get_by_course_id(db, course_id, business_id)


def get_certificates_by_id_number(db: Session, id_number: str, business_id: int):
    id_number_hash = blind_index.generate_blind_index(id_number)
    return mdt_certificate_repo.get_by_id_number_hash(db, id_number_hash, business_id)


def delete_certificate(db: Session, certificate_id: int, business_id: int):
    with db.begin():
        certificate = get_certificate_by_id(db, certificate_id, business_id)

        if not certificate:
            raise MDTCertificateNotFoundError("Certificado no encontrado")

        mdt_certificate_repo.delete_soft_by_id(db, certificate_id)
...
def get_certificate_by_id_number_and_course(
    ...
    with db.begin():
        ...
        if not certificate:
            raise MDTCertificateNotFoundError("No se encontró el certificado buscado.")
