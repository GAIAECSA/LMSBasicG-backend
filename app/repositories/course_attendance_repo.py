...
def get_by_id(db: Session, certificate_id: int):
    return db.query(CourseAttendance).filter(CourseAttendance.id == certificate_id, CourseAttendance.deleted == False).first()

def get_by_course(db: Session, course_id: int):
    return db.query(CourseAttendance).filter(CourseAttendance.course_id == course_id, CourseAttendance.deleted == False).all()